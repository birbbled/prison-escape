...
public class Guard {
	...
	protected guardResource guardResource; // This object holds all of the images that will be used to draw the Sprite.
	...
	public Guard(int x_coordinate, int y_coordinate){
		...
		guardResource = new guardResource("images/robot/", 4, 70);
		jumpCounter = -1;
	...
	public Rectangle getBounds(){
		if(x_direction < 0)
			return new Rectangle(x_coordinate + guardResource.getImageOffset(), y_coordinate, 
					guardResource.getImage().getIconWidth() - guardResource.getImageOffset()/2+15, 
					guardResource.getImage().getIconHeight());
		else
			return new Rectangle(x_coordinate + guardResource.getImageOffset() -12, y_coordinate, 
					guardResource.getImage().getIconWidth() - guardResource.getImageOffset()/2+30, 
					guardResource.getImage().getIconHeight());
	}
	...
	public void move(Component c){
		...
		if (!isDead && ((x_coordinate > - (2*guardResource.getImageOffset()) && x_direction == -2 || x_direction == -5) ||
				(x_coordinate + guardResource.getImage().getIconWidth() + guardResource.getImageOffset() < c.getWidth() && (x_direction == 2 || x_direction == 5) )))
			x_coordinate += (x_direction);
		// jump
		else if (!isDead && (y_coordinate > 0 && y_direction == -1) || 
				(y_coordinate + guardResource.getImage().getIconWidth() < c.getHeight() && y_direction == 1 ))
		...
		guardResource.updateImage(x_direction + y_direction, jumpCounter >= 0, isDead);
	}
	...
	public void draw(Graphics g, Component c) {
		...
		if(x_direction < 0)
			g2.drawImage(guardResource.getImage().getImage(), x_coordinate + guardResource.getImage().getIconWidth() + guardResource.getImageOffset(), 
					y_coordinate, -guardResource.getImage().getIconWidth(), guardResource.getImage().getIconHeight(), null);
		else
			g2.drawImage(guardResource.getImage().getImage(), x_coordinate + guardResource.getImage().getIconWidth(), 
					y_coordinate, guardResource.getImage().getIconWidth(), guardResource.getImage().getIconHeight(), null);
	}
}